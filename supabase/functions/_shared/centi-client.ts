...
const CENTI_BASE = "https://acessoainformacao.piracanjuba.go.leg.br";
...
export async function centiCall<T = unknown>(
  ...
  const resp = await fetch(`${CENTI_BASE}/api`, {
    method: "POST",
    headers: {
      ...
      "Origin": CENTI_BASE,
      "Referer": referer.startsWith("http") ? referer : `${CENTI_BASE}${referer}`,
      "User-Agent":
...
export async function centiList<T = unknown>(
  referer: string,
  acao: string,
  opts?: { extra?: Record<string, string>; offset?: number; pageSize?: number },
): Promise<T[]> {
...
export async function centiListAll<T = unknown>(
  referer: string,
  acao: string,
  opts?: { extra?: Record<string, string>; pageSize?: number; maxPages?: number },
): Promise<T[]> {
