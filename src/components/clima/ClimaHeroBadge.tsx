...
import { Cloud, CloudRain, Sun, CloudSun, CloudDrizzle, Droplets } from "lucide-react";
import { fetchClimaUltimoDia } from "@/lib/data/clima";

function iconForChuva(precip: number | null) {
  if ((precip ?? 0) > 5) return CloudRain;
  if ((precip ?? 0) > 1) return CloudDrizzle;
...
export default async function ClimaHeroBadge() {
  ...
  return (
    ...
    >
      <Icon className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 text-amber-300 shrink-0" aria-hidden />
