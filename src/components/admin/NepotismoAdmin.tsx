...
function Chips({ items, tone }: { items: string[]; tone: "muted" | "match" }) {
  ...
  return (
    <span className="inline-flex flex-wrap gap-1">
      {items.map((s) => (
        <span key={s} className={`rounded px-1.5 py-0.5 text-xs font-medium border ${cls}`}>
          {s}
        </span>
      ))}
    </span>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <div className="space-y-1">
            <Label htmlFor="nepo-senha" className="text-sm flex items-center gap-1">
          ...
          {data.resultado.map((a) => (
            <div key={a.agente + a.cargo} className="rounded-xl border border-border bg-card p-4 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="secondary" className="text-xs">{a.cargo}</Badge>
              ...
              <ul className="space-y-1.5">
                {a.vinculos.map((v, i) => (
                  <li
                    key={`${v.nome}-${i}`}
                    className="flex flex-wrap items-center gap-2 rounded-lg bg-muted/40 px-2.5 py-1.5 text-sm"
                  >
                    <ArrowRight className="w-3 h-3 text-muted-foreground shrink-0" />
                    <span className="font-medium text-foreground">{v.nome}</span>
                    <span className="text-muted-foreground">{v.cargo}</span>
                    <Badge variant="outline" className="text-xs">{v.fonte}</Badge>
