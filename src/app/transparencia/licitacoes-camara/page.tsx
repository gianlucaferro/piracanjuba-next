  ...
  return (
    <div className="container py-6 md:py-10 max-w-5xl space-y-6">
      ...
      {stats && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="stat-card">
              <p className="text-[10px] uppercase text-muted-foreground tracking-wider">Estimado total</p>
              <p className="text-sm font-extrabold text-foreground mt-0.5">{fmtBRL(stats.total_estimado)}</p>
            </div>
            <div className="stat-card">
              <p className="text-[10px] uppercase text-muted-foreground tracking-wider">Homologado total</p>
              <p className="text-sm font-extrabold text-foreground mt-0.5">{fmtBRL(stats.total_homologado)}</p>
            </div>
            <div className="stat-card">
              <p className="text-[10px] uppercase text-muted-foreground tracking-wider">Modalidades</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {stats.por_situacao.slice(0, 10).map((s) => (
                <div key={s.situacao} className="flex items-center gap-2 text-sm">
                  <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${SITUACAO_COR[s.situacao] ?? "bg-slate-500/15 text-slate-700"}`}>
                    {s.situacao}
                  </span>
      ...
      <section className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">Últimas {licits.length} licitações (de {stats?.total ?? 0} totais).</p>
        {licits.map((l) => (
          <article key={l.id} className="stat-card hover:border-purple-500/30 transition-colors">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {l.modalidade && (
                  <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-700">
                    {l.modalidade}
                  </span>
                )}
                {l.situacao && (
                  <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${SITUACAO_COR[l.situacao] ?? "bg-slate-500/15 text-slate-700"}`}>
                    {l.situacao}
                  </span>
                )}
              </div>
